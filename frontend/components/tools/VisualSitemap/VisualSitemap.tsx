...
const customControlStyles = `
  /* 🛑 CRITICAL FIX: Ensures NO scrollbars are generated by the browser */
  body {
  ...
  /* Ensures the React Flow container and pane stretch fully */
  .react-flow {
  ...
  .custom-node-link:hover { opacity: 1; }

  /* Custom Zoom Controls Styling (Octopus-inspired design) */
  ...
  .custom-zoom-controls .zoom-percentage {
    padding: 6px 4px;
    min-width: 40px;
    text-align: center;
    display: flex;
  ...
  /* Bottom-right small zoom control */
  .visual-zoom-control {
    display: flex;
    align-items: center;
    gap: 8px;
  ...
  return (
    <div className="relative w-full h-full"> 
      ...
      >
        ...
        {/* Bottom-center action toolbar (mimics provided screenshots) */}
        <Panel
        ...
        {/* Bottom-right zoom control (compact) */}
        <Panel
          position="bottom-right"
          className="z-40"
...
function VisualSitemapContent() {
  ...
  const renderNodeLabel = useCallback((title: string | undefined, url: string | undefined, level: number, highlight = false, mode: string = 'default') => {
    const colors = getMapStyles(mode, level)
    const borderStyle = highlight ? `2px solid ${colors.accent ?? '#2563eb'}` : `1px solid ${colors.border}`
    const boxShadow = highlight ? `0 8px 28px ${colors.accent ? 'rgba(6,182,212,0.12)' : 'rgba(37,99,235,0.12)'}` : 'none'

    return (
      <div
        className="custom-node"
        style={{ width: nodeWidth, border: borderStyle, boxShadow }}
      >
        <div
          style={{
        ...
        >
          <span style={{ fontWeight: 600, fontSize: '13px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', paddingRight: '8px' }}>
            {title || 'Untitled Page'}
          </span>
                <a
                  href={url}
        ...
        <div
          style={{
            padding: '8px 10px',
            fontSize: '10px',
                  color: colors.urlText ?? colors.text,
            wordBreak: 'break-all',
                  background: colors.bg,
            minHeight: 30,
          }}
  ...
  const convertToFlowData = useCallback(
    (data: { pages?: SitemapNode[] }, layout: 'default' | 'vertical' = 'default', mode: string = 'default'): { nodes: Node[]; edges: Edge[] } => {
  ...
  const currentNodeId = `node-${nodeId++}`
      // colors intentionally not used here (styles applied via renderNodeLabel)
        ...
        flowNodes.push({
          ...
          data: {
            ...
            title: node.title,
            highlight: false,
            label: renderNodeLabel(node.title, node.url, level, false, mode),
          },
          position: { x: xOffset, y: yOffset },
          style: { padding: 0, border: 'none', borderRadius: '8px', background: 'transparent', width: nodeWidth },
          draggable: true,
        })
        ...
        if (node.children?.length) {
          ...
          if (layout === 'vertical') {
            // For vertical layout, siblings are arranged horizontally, depth increases Y
            const startX = xOffset - totalWidth / 2
            node.children.forEach((child, i) =>
              processNode(child, currentNodeId, level + 1, startX + i * childSpacing, yOffset + 240)
            )
          } else {
            const startX = xOffset - totalWidth / 2
            node.children.forEach((child, i) =>
              processNode(child, currentNodeId, level + 1, startX + i * childSpacing, yOffset + 160)
            )
          }
        }
        return xOffset
      ...
      if (data.pages?.length) {
        ...
        if (layout === 'vertical') {
          // stack top-level pages vertically
          const startY = - (totalWidth / 2)
          data.pages.forEach((page, i) =>
            processNode(page, null, 0, 0, startY + i * pageSpacing)
          )
        } else {
  ...
  const generateVisual = async (): Promise<void> => {
    ...
    try {
      ...
      // store raw sitemap data so we can re-layout or recolor later
      setSitemapRaw(data)
  ...
  const getExportPayload = useCallback(async (format: string) => {
    // support svg/png captures; for text exports, serialize sitemapRaw if available
  ...
  // Control the ReactFlow pane background separately (mapMode 'dark' should darken the board, not the nodes)
  useEffect(() => {
    try {
      if (mapMode === 'dark') {
        // dark board background (regardless of node visuals)
  ...
  const performSearch = useCallback(() => {
    ...
    if (reactFlowInstance) {
      ...
      const h = (match.height as number) || 120
  ...
  // When layout changes (or sitemapRaw updated), recompute positions
  useEffect(() => {
  ...
  >
      ...
      {/* URL Input Box (Conditional display) */}
      {!isGenerated && !loading && (
