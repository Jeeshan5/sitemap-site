...
function VisualSitemapContent() {
  ...
  const calculateNodeSize = useCallback((inboundLinks: number, allLinks: number[]): number => {
    const MAX_WIDTH = 350
    const MIN_WIDTH = 200
    if (inboundLinks === 0) return MIN_WIDTH
    const maxLinks = Math.max(1, ...allLinks)
    const logMax = Math.log10(maxLinks + 1)
    const logCurrent = Math.log10(inboundLinks + 1)
    const scaleFactor = logMax > 0 ? logCurrent / logMax : 0
    return MIN_WIDTH + (MAX_WIDTH - MIN_WIDTH) * scaleFactor
  }, [])
  ...
  const convertToFlowData = useCallback((data: { pages?: SitemapNode[] }, mode: string = 'default'): { nodes: Node[]; edges: Edge[] } => {
      ...
      if (parentId) {
        flowEdges.push({
          ...
          style: {
            stroke: '#cbd5e1',
            strokeWidth: 2,
            strokeDasharray: node.isCollapsed ? '5 5' : 'none',
          },
          markerEnd: { type: MarkerType.ArrowClosed, color: '#cbd5e1', width: 15, height: 15 },
        })
      }

      if (node.children?.length && !node.isCollapsed) {
        const visibleChildren = node.children
        const totalWidth = (visibleChildren.length - 1) * siblingSpacing
    ...
    if (data.pages?.length) {
      const pageSpacing = siblingSpacing
